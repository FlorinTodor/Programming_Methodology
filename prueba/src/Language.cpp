...
double Language::getDistance(const Language &otherLanguage) const {
    ...
    int rank2=0; //Posición en L2

    if (this->_size == 0){
        throw std::invalid_argument("Bigrama vacio");
    }

    for(int i=0; i< _size; i++){//Sólo tenemos que comprobar los elementos de l1, por eso solo hasta _size de l1
        Bigram aux = _vectorBigramFreq[i].getBigram();
