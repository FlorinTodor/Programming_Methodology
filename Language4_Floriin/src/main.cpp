...
int main(int argc, char* argv[]) {
    ...
    if(argc <= 1){ //Solo se el envía el ejecutable
            return 0;
        }
    ...
    if(string(argv[1]) == "-t"){
       ...
       if(argc <=2){ //Debido a que no se puede realizar nada
            return 0;
        }
        ...
        if(max){
            ...
            cout<<"Identifier of the farthest language: "<<languages[pos].getLanguageId()<<endl;
        ...
        if(argc <=2){ //Debido a que con un solo fichero no se puede hacer nada
            return 0;
        }
